...
export class PolicyEngine {
  ...
  evaluate(action: PolicyAction): PolicyEvaluation {
    ...
    if (this.sessionAllowlist.has(action.payload)) {
      return { decision: 'ALLOW', riskScore: 0, riskLevel: 'low', reason: 'Session allowlist', timestamp: Date.now(), actionId: id };
    }

    // 2. Hard deny
    if (this.config.denylistPatterns.some(p => this.match(action.payload, p))) {
      return { decision: 'DENY', riskScore: 100, riskLevel: 'critical', reason: 'Denied by policy', timestamp: Date.now(), actionId: id };
    }
